...
程序运行在 Linux 2.6 系统上，修改上述列出，令其可以纳秒级精度来显示文件时间戳。处理 `timespec` 结构体。具体在 [[时间]] 章节介绍
...
int main(const int argc, char *argv[]) {
    ...
    if (argc != 3) {
        fprintf(stderr, "Usage: %s <pathname> <perms:f r w x>\n", argv[0]);
        return 1;
    }
...
下面编写一个函数， **根据进程的 _有效_ 用户和组 ID 来进行权限检查** ^[[[进程凭证]]] 
