# 作业34：[[System V 信号量]] 和 [[System V 共享内存]]

## 编程题

使用 [[System V 信号量]] 将 [[System V 共享内存#使用共享内存]] 中的示例代码修改为使用信号量同步

为了保证一次只有一个进程访问共享内存段，这里我们使用 [[System V 信号量]] 作为同步工具。这里我们需要使用两个信号用于控制读者和写者
