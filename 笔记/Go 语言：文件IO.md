...
+ 参数 `prem` 代表文件的 **模式** 和 **权限位**。这些位在所有系统上具有相同的定义，因此有关文件的信息可以便携地从一个系统移动到另一个系统。并非所有位都适用于所有系统。目录中唯一需要的位是 `ModeDir`。`prem` 的类型是 `os.FileMode`，它是 `fs.FileMode` 的别名，本质上是 `uint32`  
