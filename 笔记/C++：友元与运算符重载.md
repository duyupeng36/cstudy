...
重载二元运算符的方式有两种方式：**接收一个参数的非 static 成员函数** 和 **接收两个参数的非成员函数**
...
重载一元运算符的方式也有两种：**不接收参数的非 static 成员函数** 和 **接收一个参数的非成员函数**
...
在 [[C++：抽象机制概览]] 中我们定义了一个 `complex` 类，下面我们逐步完善这个类。这里我们依旧使用 `double` 作为基础类型，因此它与 `std::complex<double>` 几乎等价。下面给出了 `complex` 类的详细设计
