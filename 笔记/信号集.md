...
> 
> + `SIG_BLOCK`: 将 `set` 指向的信号集添加到进程当前信号掩码中。即，将信号掩码设置为其当前值和 `set` 的并集
...
如果某进程接受了一个该进程正在阻塞的信号，那么会将该信号添加到进程的 **等待信号集**中。当解除了对该信号的阻塞时，会随之将信号传递给此进程。
...
| `sa_flags`     | 描述                      |
| :------------- | ----------------------- |
| `SA_NOCLDSTOP` | 参考 [[监控子进程#SIGCHLD 信号]] |
