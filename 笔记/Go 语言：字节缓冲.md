...
标准包 `bytes` 提供了对 **字节切片** 进行读写操作的一系列函数，字节切片处理的函数比较多分为基本处理函数、比较函数、后缀检查函数、索引函数、分割函数、大小写处理函数和子切片处理函数等. 与 [[Go 语言：字符串#标准包 `strings`]] 中的方法具有相似的含义，只是操作的是 `byte` 切片 


## `bytes.Buffer` 类型

`bytes.Buffer` 是一个**大小可变的字节缓冲区**，实现了 `Buffer.Read()` 和 `Buffer.Write()` 方法。`Buffer` 的零值表示缓冲区为空，可以随时使用
...
> [!warning] 
> 
> 此时 `bytes.Buffer` 对象拥有 `buf` 的所有权，调用者在调用 `NewBuffer` 之后不应再使用 `buf`
> 
...
> [!tip] 
> 
> 在大多数情况下，`new(bytes.Buffer)`（或仅仅声明一个 `bytes.Buffer` 变量）就足以初始化一个 `Buffer`
> 
...
`bytes.Reader` 通过从字节切片中读取数据来实现 `io.Reader`、`io.ReaderAt`、`io.WriterTo`、`io.Seeker`、`io.ByteScanner` 和 `io.RuneScanner` 接口。与 `bytes.Buffer` 不同，`bytes.Reader` 类型对象是只读的，并且支持寻道。`Reader` 类型的零值操作类似于读取空片段
