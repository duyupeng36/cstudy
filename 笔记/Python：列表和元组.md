...
**列表和字符串有很多共性**，例如，**索引** 和 **切片** 操作。这两种数据类型是 **序列** 。随着 Python 语言的发展，其他的序列类型也被加入其中。**元组** 是另一种标准序列类型
...
>>> t[0]
12345
>>> u = t, (1,2,3,4,5)  # 元组可以包含元组
>>> u
((12345, 54321, 'hello!'), (1, 2, 3, 4, 5))
>>> t[0] = 8888   # 元组是不可变类型，不能修改元组元素的引用
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
TypeError: 'tuple' object does not support item assignment
>>> v = ([1,2,3],[3,2,1])  # 元组可以包含列表
>>> v
([1, 2, 3], [3, 2, 1])
```
...
圆括号里只有一个值的话不够明确。丑陋，但是有效。例如：

```python
>>> empty = ()
>>> singleton = 'hello',   # 这个逗号是必须的
>>> len(empty)
0
>>> len(singleton)
1
...
现在，可以解释在 [[Python：基础语法#while 语句]] 的例子中使用的 **赋值语句  `a, b = b, a + b`  其实就是元组的打包与解包**
...
> [!important] 使用 `()` 创建只有一个元素的元祖，一定要在元素后加上逗号
...
>>> t2
[1, 1, 1, 1]
>>> # 创建一个元素的元祖时，必须要在元素后补上一个逗号
>>> t3 = ([1],) * 4
>>> t3
([1], [1], [1], [1])
```

> [!success] 理解可变类型与不可变类型
> **元组的不可变性仅 _针对元组中的引用_ 而言**
> 
