...
|                    |                                                     |
| Linux 系统调用         |                                                     |
| [[文件 IO：通用 IO 模型]] | 系统调用 `open` `close` `read` `write` `lseek`  `ioctl` |
| [[深入文件 IO]]        | 原子操作，系统调用 `fcntl`  `dup` `dup2` `dup3`              |
| [[文件 IO 缓冲]]       | 同步 IO 完整性；系统调用 `fsync` `fdatasync` `sync`           |
| [[文件属性]]           | 系统调用 `stat` `utime` `access` `umask` `chmod`        |
| [[目录与链接]]          |                                                     |
|                    |                                                     |
|                    |                                                     |
|                    |                                                     |
